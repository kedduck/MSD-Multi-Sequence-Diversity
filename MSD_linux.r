...
filename = print(Args[1])
align_file <- readAAStringSet(filename)
...
####保存结果文件
write.csv(NVI_data, file = paste(filename, "NVI.csv",sep = '_'), row.names = FALSE)
write.table(NVI_static, file = paste(filename, "static.txt",sep = '_'), 
          row.names = FALSE)
#write.csv(NVI_average_data, file = "NVI_average.csv", row.names = FALSE)

#####NVI画图
pdf(file=paste(filename, "NVI.pdf",sep = '_'))
...
####保存结果文件
write.csv(mix_matrix, file = paste(filename, "iden_sim.csv",sep = '_'))
#####画图
pdf(file=paste(filename, "iden_sim.pdf",sep = '_'))
cor.plot <- corrplot(corr = mix_matrix, method="color",
                     is.corr = FALSE, col.lim = c(min(mix_matrix),1),
